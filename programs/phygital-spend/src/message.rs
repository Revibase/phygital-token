...
/// exact bytes (via `phygital_token::verify_asset`), binding the spend to a specific recipient,
/// mint, and amount. The spend program reconstructs them from its own accounts/args and requires a
/// matching `verify_asset` in the same transaction.
