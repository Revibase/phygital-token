...
use litesvm::LiteSVM;
use phygital_token::constants::PROGRAM_AUTHORITY_SEED;
use phygital_token_hook::accounts::ExecuteTransferHook;
...
fn deploy_hook(svm: &mut LiteSVM) {
    let program_id = phygital_token_hook::ID;
    let manifest_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"));
    let candidates = [
        manifest_dir.join("../../target/deploy/phygital_token_hook.so"),
        manifest_dir.join("../../target/sbpf-solana-solana/release/phygital_token_hook.so"),
    ];
    let bytes = candidates
        .iter()
        .find_map(|path| std::fs::read(path).ok())
        .expect("phygital_token_hook artifact not found — run anchor build");
...
fn hook_ix(authority: Pubkey) -> Instruction {
    ...
    Instruction {
        program_id,
        accounts,
        data: phygital_token_hook::instruction::ExecuteTransferHook { amount: 1 }.data(),
    }
}
...
fn hook_accepts_program_authority_pda() {
    ...
    let (program_authority, _) =
        Pubkey::find_program_address(&[PROGRAM_AUTHORITY_SEED], &phygital_token::ID);
