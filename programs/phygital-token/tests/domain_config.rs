...
fn create_domain_config_initializes_allowlist() {
    ...
    let config = phygital_token::state::DomainConfig::try_deserialize(&mut account.data.as_ref())
...
fn update_domain_config_transfers_authority() {
    ...
    let ix = ctx.update_domain_config_ix_with_args(
        ctx.payer.pubkey(),
        TEST_RP_ID,
        phygital_token::UpdateDomainConfigArgs {
...
fn create_domain_config_allows_designated_authority_to_mint() {
    ...
    let mint = TestContext::create_mint(
        ...
        &owner,
        &group,
        common::sample_create_mint_args(),
    );

    let passkey = TestPasskey::generate();
    let secp256r1_pubkey = phygital_token::Secp256r1Pubkey(passkey.compressed_pubkey);
