...
use common::{
    assert_phygital_token_program_error, assert_transaction_failed, current_slot_entry,
...
#[test]
fn set_linked_wallet_rejects_wrong_signature() {
...
#[test]
fn set_linked_wallet_rejects_passkey_for_different_token() {
    let mut ctx = TestContext::new();
    ...
    let transfer_ix = ctx.set_linked_wallet_ix(
        recipient.pubkey(),
        phygital_token_b.phygital_token,
        verify_args,
        slot_number,
    );
    ctx.svm
...
#[test]
fn set_linked_wallet_succeeds_when_secp_not_immediately_preceding() {
    let mut ctx = TestContext::new();
    let passkey = TestPasskey::generate();
    let phygital_token = ctx.init_phygital_token(&passkey);
    let recipient = Keypair::new();
    ...
    ctx.svm
        .airdrop(&recipient.pubkey(), 2 * LAMPORTS_PER_SOL)
        .ok();
    ctx.send_set_linked_wallet_with_instructions(vec![secp_ix, noop, transfer_ix], &[&recipient])
...
#[test]
fn set_linked_wallet_rejects_slot_not_in_sysvar() {
    let mut ctx = TestContext::new();
    let passkey = TestPasskey::generate();
    let phygital_token = ctx.init_phygital_token(&passkey);
    let recipient = Keypair::new();
    ...
    let err = ctx.send_set_linked_wallet_at_slot(
        &phygital_token,
        &recipient,
        true,
        Some(missing_slot),
