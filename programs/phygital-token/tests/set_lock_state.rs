...
fn set_lock_state_owner_can_toggle_lock() {
    ...
    // Claiming a Controlled phygital_token auto-locks it.
    assert_eq!(ctx.phygital_token_lock_state(phygital_token.phygital_token), true);

    let unlock_ix = ctx.set_lock_state_ix(holder.pubkey(), phygital_token.phygital_token, false);
    TestContext::send_instruction(&mut ctx.svm, unlock_ix, &[&holder]).expect("unlock phygital_token");
...
fn set_lock_state_rejects_non_owner() {
    ...
    let ix = ctx.set_lock_state_ix(attacker.pubkey(), phygital_token.phygital_token, false);
...
fn set_lock_state_rejects_non_lockable_token() {
    ...
    let ix = ctx.set_lock_state_ix(holder.pubkey(), phygital_token.phygital_token, true);
    let err = TestContext::send_instruction(&mut ctx.svm, ix, &[&holder]);
    assert_phygital_token_program_error(err, "TokenIsNotLockable");
    assert_eq!(ctx.phygital_token_lock_state(phygital_token.phygital_token), false);
}
...
fn locked_holder_cannot_transfer() {
    ...
    let err = ctx
        .send_transfer_ownership(&phygital_token, &next_recipient, true)
