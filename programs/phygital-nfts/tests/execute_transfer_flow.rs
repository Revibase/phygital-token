...
use common::{MintedCard, TestContext, TestPasskey, LAMPORTS_PER_SOL};
...
fn execute_transfer_rejects_sender_not_matching_card_owner() {
    ...
    assert_eq!(
        ctx.token_balance(recipient.pubkey(), card_for_transfer.mint),
        1
    );
    assert_eq!(
        ctx.token_balance(wrong_recipient.pubkey(), card_for_transfer.mint),
        0
    );
}
...
fn execute_transfer_keeps_sender_ata_when_balance_remains() {
    ...
    assert_eq!(
        ctx.token_balance(program_authority, card_for_transfer.mint),
        1
    );
    assert!(
...
fn execute_transfer_requires_preceding_secp256r1_instruction() {
    ...
    assert!(
        err_str.contains("InvalidSecp256r1Instruction")
            ...
            || err_str.contains("InvalidTapMessage"),
        "unexpected error: {err:?}"
    );
...
fn direct_owner_transfer_checked_is_blocked_by_transfer_hook() {
    ...
    let create_ata_ix =
        ctx.create_recipient_ata_ix(recipient.pubkey(), recipient.pubkey(), card.mint);
    TestContext::send_instruction(&mut ctx.svm, create_ata_ix, &[&recipient])
        .expect("create recipient ata");

    let transfer_ix = ctx.owner_transfer_checked_ix(holder.pubkey(), card.mint, recipient.pubkey());
...
#[test]
fn execute_transfer_rejects_counter_not_greater_than_last_transfer() {
    let mut ctx = TestContext::new();
    let passkey = TestPasskey::generate();
    let card = ctx.mint_card_with_passkey(&passkey);
    ...
    // Replaying the same tap counter after a successful transfer must be rejected.
    let err = ctx
        .send_execute_transfer_from(
            &card,
        ...
        .expect_err("reusing the same counter after a successful transfer should fail");

    let err_str = format!("{err:?}");
    assert!(
        err_str.contains("StaleTransferCounter") || err_str.contains("6011"),
        "expected stale counter error, got: {err:?}"
    );
...
#[test]
fn execute_transfer_allows_next_transfer_with_higher_counter() {
    let mut ctx = TestContext::new();
    let passkey = TestPasskey::generate();
    let card = ctx.mint_card_with_passkey(&passkey);
    let first_recipient = Keypair::new();
    ...
    .expect("second transfer with a higher counter should succeed");
