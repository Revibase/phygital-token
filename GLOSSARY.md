...
| **Owner** | `phygital_token.owner` | Current custodian. Set at `initialize` and updated by `transfer_ownership`. |
| **Mint** | `phygital_token.mint` | Optional SPL mint binding. Starts as the default pubkey until `set_mint`. |
| **Token type** | `phygital_token.token_type` | `Controlled` (auto-locks after claim; must forfeit via `remove_ownership` before next transfer) or `Bearer` (freely re-transferable). |
...
|-------------|--------------|
| `initialize` | Creates a **phygital_token** PDA seeded by `secp256r1_pubkey`, stores `identifier`, `token_type`, and `owner`. Restricted to `ADMIN`. |
...
| `remove_ownership` | Wallet-signed forfeiture — resets `phygital_token.owner` to default and clears `is_locked`. |
