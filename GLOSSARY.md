...
PhygitalToken (phygital_token PDA)  ← created by `initialize`
  ...
  ├── owner                ← wallet pubkey set at initialize (or updated by transfer_ownership)
...
| **Owner** | `phygital_token.owner` | Current custodian. Set at `initialize`. Updated by `transfer_ownership` / cleared by `remove_ownership` for Bearer and Controlled; immutable for Permanent. |
| **Mint** | `phygital_token.mint` | Optional SPL mint binding. Starts as the default pubkey until `set_mint`. |
| **Token type** | `phygital_token.token_type` | `Permanent` (discriminant 0 — owner required at initialize; never transferable or forfeitable; remaps former Controlled accounts), `Bearer` (freely re-transferable), or `Controlled` (auto-locks after claim; must forfeit via `remove_ownership` before next transfer). |
...
|-------------|--------------|
| `initialize` | Creates a **phygital_token** PDA seeded by `secp256r1_pubkey`, stores `identifier`, `token_type`, and `owner`. Permanent requires a non-default `owner`. Restricted to `ADMIN`. |
| `set_mint` | Admin binds an SPL mint pubkey onto `phygital_token.mint`. Restricted to `ADMIN`. |
| `transfer_ownership` | Passkey-authorized ownership update to `recipient` (no SPL token). Rejected for Permanent. Controlled tokens must be unlocked and re-lock after claim. |
| `verify` | Passkey-authorized message proof; optional `expected_rp_id` / `expected_origins` (`Option`); updates `last_sign_count`. |
| `remove_ownership` | Wallet-signed forfeiture — resets `phygital_token.owner` to default and clears `is_locked`. Rejected for Permanent. |
