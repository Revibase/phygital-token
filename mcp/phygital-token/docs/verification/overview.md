...
Use `beginTransfer({ rpc, phygitalToken, rpId? })` → `authenticatePasskeyForTransfer` → `completeTransfer`. Optional `rpId` defaults to `window.location.hostname`. The passkey comes from `response.id` at complete time. That builds `secp256r1_verify` + `transfer_ownership`, which updates `phygital_token.owner` only (no SPL token).
