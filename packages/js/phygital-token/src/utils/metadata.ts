 ...
 * `linked_wallet` 32 | `mint` 32 | `last_sign_count` 4 | `token_type` 1 | `is_locked` 1
...
/** memcmp offset of `linked_wallet` in account data. */
const PHYGITAL_TOKEN_LINKED_WALLET_OFFSET = DISCRIMINATOR_LEN;
/** memcmp offset of `mint` in account data. */
const PHYGITAL_TOKEN_MINT_OFFSET =
  PHYGITAL_TOKEN_LINKED_WALLET_OFFSET + PUBKEY_LEN;
...
/**
 * List tokens linked to `linkedWallet` (via `getProgramAccounts` memcmp).
 * `rpc` and `linkedWallet` are Kit types (`Rpc`, `Address`).
 */
export async function fetchPhygitalTokensByLinkedWallet(
  rpc: Rpc<SolanaRpcApi>,
    ...
    .getProgramAccounts(PHYGITAL_TOKEN_PROGRAM_ADDRESS, {
      ...
      filters: [
        { dataSize: BigInt(getPhygitalTokenSize()) },
        {
          memcmp: {
            encoding: "base64" as const,
            offset: BigInt(PHYGITAL_TOKEN_LINKED_WALLET_OFFSET),
            bytes: getBase64Decoder().decode(
              getAddressEncoder().encode(linkedWallet),
